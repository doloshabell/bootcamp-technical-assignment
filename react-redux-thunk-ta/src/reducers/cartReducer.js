...
const cartReducer = (state = initState, action) => {
  ...
  if (action.type === ADD_TO_CART) {
    // Add code here
    let itemToAdd = state.items.find((item) => (
      item.id === action.payload
    ))

    let itemExist = state.addedItems.find((item) => (
    ...
    if (itemExist) {
      ...
      itemToAdd.quantity = 1;
      let newTotalPrice = state.total + itemToAdd.price
  ...
  if (action.type === REMOVE_ITEM) {
    // Add code here
    let itemToRemoved = state.addedItems.find((item) => (
      action.payload === item.id
    ))

    let itemHasRemoved = state.addedItems.filter((item) => (
      action.payload !== item.id
    ))

    let newTotalPrice = state.total - (itemToRemoved.price * itemToRemoved.quantity)
    
    return {
      ...state,
      addedItems: itemHasRemoved,
      total: newTotalPrice
    }
  }
  ...
  if (action.type === SUB_QUANTITY) {
    // Add code here
    let itemToSub = state.items.find((item) => (
      item.id === action.payload
    ));

    if (itemToSub.quantity === 1) {
      let newItemQuantity = state.addedItems.filter((item) => (
        item.id !== action.payload
      ));

      let newTotalPrice = state.total - itemToSub.price;

      return {
        ...state,
        addedItems: newItemQuantity,
        total: newTotalPrice,
      }
    }
  ...
  if (action.type === ADD_SHIPPING) {
    ...
    let newTotalPrice = state.total + 6;
    return{
      ...state,
      total: newTotalPrice,
    }
